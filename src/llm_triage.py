...
def _build_prompt(
    ...
    return (
        ...
        "Before comparing candidates, identify internally from the alert evidence:\n"
        "- the action that was observed\n"
        "- the object or resource acted upon\n"
        "- the process or tool that performed the action\n"
        "- the command or operation that was executed\n"
        "- the result or outcome directly supported by the telemetry\n"
        "Do not infer an additional action, objective, mechanism, or outcome that the "
        "alert evidence does not show.\n\n"
        "Candidate comparison rubric:\n"
        "Evaluate every candidate internally using the following evidence:\n"
        "- Determine whether the candidate's defining behavior directly matches the "
        "observed action and object.\n"
        "- Compare the command and process with the behavior required by the candidate.\n"
        "- Compare the detection_logic and alert description with the candidate "
        "description_summary.\n"
        ...
        "- Reject candidates whose defining behavior requires evidence not shown by "
        "the alert.\n"
        "- Prefer a directly evidenced behavior over a broader category, related "
        "mechanism, possible consequence, or adjacent behavior.\n"
        "- Among supported candidates, choose the one that explains the strongest direct "
        "evidence while requiring the fewest unsupported assumptions.\n"
        "- Use specificity only as a tie-breaker when the alert contains the evidence "
        "that distinguishes the more specific candidate.\n\n"
